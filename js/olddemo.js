var loadDemoData=function(){
    ...
    var cameraHtml=function(text){
    	return '<a onclick="vm.openDialog()" style="cursor:pointer">'+text+'</a>';
    };
	...
	var room=[
		...
		{type:'Model',modelName:'door',states:{isOpen:true},position:{x:30,y:0,z:195.35},label:{text:'监室门',showDistance:300}},
		//后门
		{type:'Model',modelName:'door',states:{isOpen:false},position:{x:30,y:0,z:78.55},label:{text:'风场门',showDistance:300,position:'left'}},
	...
	var objs=[
		...
		{type:'Model',modelName:'door',states:{isOpen:false},position:{x:-25,y:0,z:266.7},label:{text:'A门',showDistance:300,position:'left'}},
		...
		{type:'Model',modelName:'door',states:{isOpen:false},position:{x:-25,y:0,z:235.7},label:{text:'B门',showDistance:300}},
		//窗
	...
	var createPassageway=function(offset,params){
		...
		//左门
		if(params.leftdoor){
			//objs.push({type:'Rect',width:0.3,height:7.6,depth:doorHeight,isOpen:false/*,dataid:'b54377945f7c4d6289c14c406f313572',dataindex:'0',label:{visible:true,text:'没有此设备信息',position:'left'}*/,map:{color:'#3385ff'},position:{x:0+offset.x,y:0+offset.y,z:210.8+offset.z},rotation:{x:-90,y:0,z:0}});
			objs.push({type:'Model',modelName:'door',states:{isOpen:false},position:{x:0+offset.x,y:0+offset.y,z:210.8+offset.z},rotation:{x:0,y:90,z:0},label:{text:roomLocationIndex+'左监区门',showDistance:300,position:'left'}});
		}
		//右门
		if(params.rightdoor){
			//objs.push({type:'Rect',width:0.3,height:7.6,depth:doorHeight,isOpen:false/*,dataid:'6b0956152bc04e77b27c1d82e418bef1',dataindex:'0',label:{visible:true,text:'没有此设备信息',position:'left'}*/,map:{color:'#3385ff'},position:{x:600+offset.x,y:0+offset.y,z:210.8+offset.z},rotation:{x:-90,y:0,z:0}});
			objs.push({type:'Model',modelName:'door',states:{isOpen:false},position:{x:600+offset.x,y:0+offset.y,z:210.8+offset.z},rotation:{x:0,y:-90,z:0},label:{text:roomLocationIndex+'右监区门',showDistance:300}});
		}
	...
	/*for(var i=0;i<objs.length;i++){
		objs[i].position.x-=250;
		objs[i].position.z-=160;
	}*/
